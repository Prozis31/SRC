...
def update_flows(flows, geoip_file, geoipasnum_file):
    ...
    flows['timestamp'] = pd.to_datetime(flows['timestamp'], unit='s', origin='unix')

    # Calculate the number of connections in a minute
    flows['num_connections'] = flows.groupby(pd.Grouper(key='timestamp', freq='1Min')).transform('count')['src_ip']
...
def filter_src_dst_dst_ip(flows):
    dst_ip_pattern = r'192\.168\.108\.[0-9]{3}$'
...
def filter_unwanted_organizations(flows):
    ...
    # Filter flows with low number of connections per 1 min (< 15)
    flows2 = flows[flows['connections_per_1min'] >= 15]
...
def filter_values_not_present(flows, protocols, country_codes, organizations, destination_ports):
    filtered_flows = flows[
        ...
        ~flows['destination_port'].isin(destination_ports)
    ]
...
src_ips = flows['src_ip'].unique()
df_src_ips = pd.DataFrame({'src_ip': src_ips})
df_src_ips.to_csv('compromised_src_ips.csv', index=False)
